...
class GettagRecursorTest(RecursorTest):
    ...
    def testEDNSOptions(self):
        ...
        expected = [
            dns.rrset.from_text(name, 0, dns.rdataclass.IN, 'A', '192.0.2.1'),
            dns.rrset.from_text_list(name, 0, dns.rdataclass.IN, 'TXT', [name, 'gettag-qtype-1', 'edns-subnet-' + subnet + '/' + str(subnetMask),
                                                                         'ednsoption-10-total-len-16',
                                                                         'ednsoption-8-total-len-8'
                                                                        ]),
            ]
        query = dns.message.make_query(name, 'A', want_dnssec=True, options=[eco,ecso])
        query.flags |= dns.flags.CD
        res = self.sendUDPQuery(query)
        self.assertResponseMatches(query, expected, res)
...
quiet=no
    ...
    function nodata(dq)
      if dq.qtype == pdns.AAAA and dq.qname:equal("nodata.luahooks.example.") then
        dq:addAnswer(pdns.AAAA, "2001:DB8::1")
        return true
      end
...
pdnslog("in nxdomain")
      if dq.qtype == pdns.A and dq.qname:equal("nxdomain.luahooks.example.") then
pdnslog("matches")
        dq.rcode=0
        dq:addAnswer(pdns.A, "192.0.2.1")
        return true
    ...
    function postresolve(dq)
      if dq.qtype == pdns.A and dq.qname:equal("postresolve.luahooks.example.") then
    ...
    function preoutquery(dq)
      if dq.remoteaddr:equal(newCA("%s.23")) and dq.qname:equal("preout.luahooks.example.") and dq.qtype == pdns.A then
        dq.rcode = -3 -- "kill"
        return true
      end
