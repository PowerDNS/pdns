...
.. changelog::
  ...
  * DNSSEC? (#6655)
  ...
  Minor features, weave paragraph from this:

  * 7094: Unique DNS Responses
  * 6823: On incoming NOTIFY load our serial from backend to have it
  * 6969: re-do of an earlier PR to add the glorious log-log histograms
  * 6997: Name threads in the programs
  * 6727: ALIAS: Respond SERVFAIL on non-NOERRORs from resolver
  * 6910: support for OpenSSL 1.1.1's ed25519 and ed448
  * 6975: Enhance query-logging with timing
  * 6913: actually truncate truncated responses
  * 6730: Implement a smarter dedup for filling packets in auth

  Upgrade notes:

  * autoserial removed
  * command line util parameters (for scripts)
  * BIND config files fixup on package upgrade
  * GOST and Botan removed
  * #6633?

  Possible tags we can use below: Bug Fixes, Improvements, Internals, API, Packages, PostgreSQL, LDAP, MySQL, Tools, DNSSEC, Removed Features, New Features, BIND, DNSUpdate, IXFRDist.
  ...
  .. change::
    :tags: Bug Fixes, IXFRDist
    :pullreq: 7225

    Preserve the correct TTL for SOA records
  ...
  .. change::
    ...
    After this PR only the opendbx backend is capable of doing autoserial.  Autoserial is implement in the backend itself and not in core.
  ...
  .. change::
    ...
    Print the content of pdns.log when the regression tests fail.
  ...
  .. change::
    ...
    ``scopeMask`` in the ``SOAData`` structure is unused after #5512.
  ...
  .. change::
    :tags: Bug Fixes, Improvements, IXFRDist
    :pullreq: 7011
  ...
  .. change::
    ...
    Rename ``rtr`` to ``rte`` since it's an ``rte``. (@jsoref)
  ...
  .. change::
    ...
    Fix TTL, optimize the construction of answers.
  ...
  .. change::
    ...
    Reduce memory usage and lock contention.

  .. change::
    :tags: Bug Fixes, IXFRDist
    :pullreq: 6886

    Fix invalid buffer usage in ``getSerialFromMaster()``.
  ...
  .. change::
    :tags: Improvements, Tools, IXFRDist
    :pullreq: 6489

    Use worker threads to handle TCP connections.
  ...
  .. change::
    :tags: Bug Fixes, Tools, IXFRDist
    :pullreq: 6302

    Also respect the AXFR timeout for the chunk's length.

  .. change::
    :tags: Bug Fixes, Tools, IXFRDist
    :pullreq: 6291

    Read only the amount of bytes we need.

  .. change::
    :tags: Improvements
    :pullreq: 6100

    Make outgoing-query-address{,6} behaviour equivalent.
  ...
  .. change::
    ...
    * Adds MMDB support. Now geoip backend can be compiled without geoip support. Closes #5268
  ...
  .. change::
    ...
    Keep latest AXFR and ``--keep`` IXFR's in memory.
  ...
  .. change::
    ...
    Allow setting the inbound AXFR timeout
  ...
  .. change::
    :tags: Bug Fixes, Improvements, Tools, IXFRDist
    :pullreq: 6264

    Prevent a nullptr exception and guard reads/writes.

  .. change::
    :tags: Bug Fixes, Tools, IXFRDist
    :pullreq: 6258

    Check the serial even if verbose is off.

  .. change::
    :tags: New Features, Tools, IXFRDist
    :pullreq: 6235
  ...
  .. change::
    ...
    Always bind the results array after executing a MySQL statement.
  ...
  .. change::
    ...
    Fix dependency tracking for backends. (@cmouse)
