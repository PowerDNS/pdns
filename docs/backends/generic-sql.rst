...
::
    ...
    gmysql-basic-query=SELECT content,ttl,prio,type,domain_id,disabled,name,auth FROM records WHERE (disabled=0 or ?) and type=? and name=?
...
::
    ...
    FROM records WHERE (disabled=0 or ?) and type=? and name=?
