...
$NAME$  1D  IN  SOA ns1.example.org. hostmaster.example.org. (
    ...
    def test_export_zone_text(self):
        name, payload, zone = self.create_zone(nameservers=["ns1.foo.com.", "ns2.foo.com."], soa_edit_api="")
        # export it
        r = self.session.get(self.url("/api/v1/servers/localhost/zones/" + name + "/export"), headers={"accept": "*/*"})
    ...
    def test_access_zone_with_invalid_content(self):
        ...
        cur.execute(
            "COMMIT"
        )  # Figuring out how many hours I wasted on this test because of this missing line is left as an exercize to the reader
        cur.close()
        db.close()
