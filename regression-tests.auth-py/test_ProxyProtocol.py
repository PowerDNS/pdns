...
"""
    ...
    def setUpClass(cls):
        ...
        if cls._backend == "lmdb":
            ...
            os.system(
                "$PDNSUTIL --config-dir=configs/auth rrset replace views.test views.test SOA 'no.where no.where 1 10800 3600 604800 3600'"
            )
            os.system("$PDNSUTIL --config-dir=configs/auth zone create views.test..squint")
            os.system(
                "$PDNSUTIL --config-dir=configs/auth rrset replace views.test..squint views.test SOA 'no.where no.where 1 10800 3600 604800 3600'"
            )
