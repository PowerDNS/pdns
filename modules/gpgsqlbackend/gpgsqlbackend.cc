...
public:
  ...
  void declareArguments(const string& suffix = "") override
  {
    ...
    declare(suffix, "info-all-secondaries-query", "", "select domains.id, domains.name, domains.type, domains.master, domains.last_check, records.content, records.name from domains LEFT JOIN records ON records.domain_id=domains.id AND records.type='SOA' where domains.type in ('SLAVE', 'CONSUMER')");
    ...
    declare(suffix, "info-all-primary-query", "", "select domains.id, domains.name, domains.type, domains.notified_serial, domains.options, domains.catalog, records.content, records.name from records join domains on records.domain_id=domains.id where records.type='SOA' and records.disabled=false and domains.type in ('MASTER', 'PRODUCER') order by domains.id");
    declare(suffix, "info-producer-members-query", "", "select domains.id, domains.name, domains.type, domains.options, records.name from records join domains on records.domain_id=domains.id where domains.type in ('MASTER', 'PRODUCER') and domains.catalog=$1 and records.type='SOA' and records.disabled=false");
    ...
    declare(suffix, "get-all-domains-query", "Retrieve all domains", "select domains.id, domains.name, records.content, domains.type, domains.master, domains.notified_serial, domains.last_check, domains.account, domains.catalog, records.name from domains LEFT JOIN records ON records.domain_id=domains.id AND records.type='SOA' WHERE records.disabled=false OR $1");
