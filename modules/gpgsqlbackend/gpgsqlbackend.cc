...
public:
  ...
  void declareArguments(const string& suffix = "") override
  {
    ...
    declare(suffix, "info-producer-members-query", "", "select domains.id, domains.name, domains.options from records join domains on records.domain_id=domains.id and records.name=domains.name where (domains.type='MASTER' or domains.type='SLAVE') and domains.catalog=$1 and records.type='SOA' and records.disabled=false");
