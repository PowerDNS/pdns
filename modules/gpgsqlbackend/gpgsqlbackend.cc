...
public:
  ...
  void declareArguments(const string& suffix = "") override
  {
    ...
    declare(suffix, "basic-query", "Basic query", record_query + " (disabled=false or $1) and type=$2 and name=$3");
    declare(suffix, "id-query", "Basic with ID query", record_query + " (disabled=false or $1) and type=$2 and name=$3 and domain_id=$4");
    declare(suffix, "any-query", "Any query", record_query + " (disabled=false or $1) and name=$2");
    declare(suffix, "any-id-query", "Any with ID query", record_query + " (disabled=false or $1) and name=$2 and domain_id=$3");
