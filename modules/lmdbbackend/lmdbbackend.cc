...
bool LMDBBackend::listInternal(const ZoneName& target, domainid_t domain_id, const std::string& match, bool include_disabled)
{
...
{
  compoundOrdername co; // NOLINT(readability-identifier-length)
  return listInternal(target, domain_id, co(domain_id), include_disabled);
}

bool LMDBBackend::listSubZone(const ZoneName& target, domainid_t domain_id)
{
  return listInternal(target, domain_id, compoundOrdername::suffixMatch(domain_id, target.operator const DNSName&()), false);
}
