...
public:
  ...
  void declareArguments( const std::string & suffix = "" )
  {
    ...
    declare(suffix, "closest-soa-query", "Closest SOA query", "SELECT content,ttl,prio,type,domain_id,disabled,name,auth FROM records WHERE disabled=0 and type='SOA' and (name=? or ? LIKE '%.' + name) ORDER BY LEN(name) DESC");
