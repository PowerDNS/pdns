...
public:
  ...
  void declareArguments(const std::string& suffix = "") override
  {
    ...
    declare(suffix, "basic-query", "Basic query", record_query + " (disabled=0 or :include_disabled) and type=:qtype and name=:qname");
    declare(suffix, "id-query", "Basic with ID query", record_query + " (disabled=0 or :include_disabled) and type=:qtype and name=:qname and domain_id=:domain_id");
    declare(suffix, "any-query", "Any query", record_query + " (disabled=0 or :include_disabled) and name=:qname");
    declare(suffix, "any-id-query", "Any with ID query", record_query + " (disabled=0 or :include_disabled) and name=:qname and domain_id=:domain_id");
