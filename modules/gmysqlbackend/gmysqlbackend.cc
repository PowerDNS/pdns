...
public:
  ...
  void declareArguments(const string& suffix = "") override
  {
    ...
    declare(suffix, "basic-query", "Basic query", record_query + " (disabled=0 or ?) and type=? and name=?");
    declare(suffix, "id-query", "Basic with ID query", record_query + " (disabled=0 or ?) and type=? and name=? and domain_id=?");
    declare(suffix, "any-query", "Any query", record_query + " (disabled=0 or ?) and name=?");
    declare(suffix, "any-id-query", "Any with ID query", record_query + " (disabled=0 or ?) and name=? and domain_id=?");
