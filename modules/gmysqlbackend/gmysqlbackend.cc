...
public:
  ...
  void declareArguments(const string& suffix = "") override
  {
    ...
    declare(suffix, "info-all-slaves-query", "", "select domains.id,domains.name,domains.master,domains.last_check,records.content FROM domains LEFT JOIN records ON records.domain_id=domains.id AND records.type='SOA' AND records.name=domains.name AND records.disabled=0 WHERE type='SLAVE'");
