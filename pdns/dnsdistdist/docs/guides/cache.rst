...
The first step is to define a cache and to assign that cache to the chosen pool, the default one being represented by the empty string:
...
.. md-tab-set::
    ...
    .. md-tab-item:: Lua
      ...
      A cache can be created with :func:`newPacketCache`, it can then be assigned to a pool with :meth:`ServerPool.setCache()`.
...
Modifying the cache at runtime
------------------------------

A reference to the cache attached to a specific pool can be retrieved with:
