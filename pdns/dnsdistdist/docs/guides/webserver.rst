...
.. http:put:: /api/v1/servers/localhost/config/allow-from
  ...
  However, restarting the :program:`dnsdist` server will restore the ALC
  to the one configured in the configuration file.

  :<json [string] value: A list of IP subnets to add to the ACL.
...
.. http:get:: /api/v1/servers/localhost/rings
  ...
  :query integer maxQueries: Return up to this many queries. When unset, return all
  :query integer maxResponses: Return up to this many responses. When unset, return all
