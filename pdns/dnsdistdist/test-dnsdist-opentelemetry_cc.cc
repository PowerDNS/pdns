...
BOOST_AUTO_TEST_CASE(getTraceID)
{
  ...
  BOOST_CHECK_NE(tracer.getTraceID(), TraceID{});
}
...
BOOST_AUTO_TEST_CASE(getLastSpanID)
{
  ...
  auto spanid = tracer.addSpan("myevent");
  lastSpanID = tracer.getLastSpanID();
  BOOST_CHECK_EQUAL(spanid, lastSpanID);

  for (auto i = 0; i < 4; i++) {
    spanid = tracer.addSpan("myevent" + std::to_string(i));
  }
  lastSpanID = tracer.getLastSpanID();
  BOOST_CHECK_EQUAL(spanid, lastSpanID);

  tracer.activate();
  spanid = tracer.addSpan("post-activation-myevent");
  lastSpanID = tracer.getLastSpanID();
  BOOST_CHECK_EQUAL(spanid, lastSpanID);

  for (auto i = 0; i < 4; i++) {
    spanid = tracer.addSpan("post-activation-myevent" + std::to_string(i));
  }
  lastSpanID = tracer.getLastSpanID();
  BOOST_CHECK_EQUAL(spanid, lastSpanID);
}
...
BOOST_AUTO_TEST_CASE(getLastSpanIDForName)
{
  ...
  // Add event before activation
  auto spanid = tracer.addSpan(eventName);
  lastSpanID = tracer.getLastSpanIDForName(eventName);
  BOOST_CHECK_EQUAL(spanid, lastSpanID);

  for (auto i = 0; i < 4; i++) {
    spanid = tracer.addSpan(eventName);
  }
  lastSpanID = tracer.getLastSpanIDForName(eventName);
  BOOST_CHECK_EQUAL(spanid, lastSpanID);
...
BOOST_AUTO_TEST_CASE(activate)
{
  ...
  // of the output.
  auto preActivationSpanID = tracer.addSpan("pre-activation-event");
  tracer.activate();
  auto postActivationSpanID = tracer.addSpan("post-activation-event");
  ...
  tracer.deactivate();
  auto preActivationSpanID2 = tracer.addSpan("pre-activation-event2");
...
BOOST_AUTO_TEST_CASE(Closer)
{
  ...
  auto prespanid = tracer.addSpan("foo");
  tracer.activate();
  auto postspanid = tracer.addSpan("bar");
  ...
  // Closer is out of scope, so the each event should have a closing time
  auto trace = tracer.getTracesData();
...
BOOST_AUTO_TEST_CASE(attributes)
{
  ...
  auto spanid = tracer.addSpan("anEvent");
...
BOOST_AUTO_TEST_CASE(getOTProtobuf)
{
  ...
  BOOST_TEST(data.size() == 31);

  tracer.activate();
  tracer.setTraceAttribute("foo", AnyValue{"bar"});
  data = tracer.getOTProtobuf();
  BOOST_TEST(data.size() == 45);
}
