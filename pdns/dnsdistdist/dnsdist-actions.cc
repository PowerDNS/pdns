...
{
public:
  SpoofSVCAction(const std::vector<SVCRecordParameters>& parameters)
  {
...
{
  return std::shared_ptr<DNSAction>(new DelayAction(delay));
}

std::shared_ptr<DNSAction> getTeeAction(const ComboAddress& rca, const std::optional<ComboAddress>& lca, bool addECS, bool addProxyProtocol)
{
...
{
  return std::shared_ptr<DNSAction>(new PoolAction(pool, stopProcessing));
}

std::shared_ptr<DNSAction> getQPSPoolAction(unsigned int limit, std::string pool, bool stopProcessing)
{
  return std::shared_ptr<DNSAction>(new QPSPoolAction(limit, pool, stopProcessing));
}
...
{
  return std::shared_ptr<DNSAction>(new SpoofSVCAction(parameters));
}
