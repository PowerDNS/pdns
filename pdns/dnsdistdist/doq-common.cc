...
static void sendFromTo(Socket& sock, const ComboAddress& peer, const ComboAddress& local, PacketBuffer& buffer, bool socketBoundToAny)
{
  const int flags = 0;
  if (local.sin4.sin_family == 0 || !socketBoundToAny) {
    // NOLINTNEXTLINE(cppcoreguidelines-pro-type-reinterpret-cast)
