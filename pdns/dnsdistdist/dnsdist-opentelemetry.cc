...
TracesData Tracer::getTracesData() const
{
  ...
  if (d_preActivationSpans != nullptr) {
  ...
  if (d_postActivationSpans != nullptr) {
...
SpanID Tracer::addSpan([[maybe_unused]] const std::string& name, [[maybe_unused]] const SpanID& parentSpanID)
{
  ...
  if (d_preActivationSpans == nullptr) {
    // This is the first time addEvent is called for this Tracer instance
    d_preActivationSpans = std::make_unique<std::vector<preActivationSpanInfo>>();
  }
...
void Tracer::setTraceAttribute([[maybe_unused]] const std::string& key, [[maybe_unused]] const AnyValue& value)
{
#ifndef DISABLE_PROTOBUF
  if (!d_activated) {
    // TODO: throw an error? Log an error? We don't want to collect any data before we are are actually tracing
    return;
  }
  if (d_attributes == nullptr) {
    d_attributes = std::make_unique<std::vector<pdns::trace::KeyValue>>();
  }
...
#ifndef DISABLE_PROTOBUF
  if (d_activated) {
    if (auto iter = std::find_if(d_postActivationSpans->rbegin(), d_postActivationSpans->rend(), [spanid](const pdns::trace::Span& span) { return span.span_id == spanid; }); iter != d_postActivationSpans->rend()) {
...
SpanID Tracer::getLastSpanID() const
{
  ...
  if (d_preActivationSpans != nullptr && d_preActivationSpans->size() != 0) {
