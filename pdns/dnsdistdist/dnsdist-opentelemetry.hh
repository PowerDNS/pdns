 ...
 * dnsdist.
 *
 * The idea is inspired by the rec-eventrace.{cc,hh} files.
 *
...
public:
  ...
  void activate()
  {
    ...
    if (d_postActivationSpans == nullptr) {
      d_postActivationSpans = std::make_unique<std::vector<pdns::trace::Span>>();
    }
#endif
  }
   ...
   * @param value
   */
  void setTraceAttribute(const std::string& key, const AnyValue& value);
  ...
  /**
   * @brief Set an attribure on a Span
   *
   * This does not work when the Tracer is not active
   *
   * @param spanID The SpanID of the Span to add the attibute to
   * @param key
   * @param value
   */
   ...
   * When the object goes our of scope, the closeSpan function is called
   */
  class Closer
  {
  public:
...
private:
  ...
  // We use pointers so we don't need to allocate any of these things if tracing is not required
  /**
   * @brief Stores all preActivationSpanInfos. It is used until d_activated is true
   */
  std::unique_ptr<std::vector<preActivationSpanInfo>> d_preActivationSpans;
  /**
   * @brief Stores all Spans. It is used when d_activated is true
   */
  std::unique_ptr<std::vector<Span>> d_postActivationSpans;
  /**
   * @brief All attributes related to this Trace
   */
  std::unique_ptr<std::vector<pdns::trace::KeyValue>> d_attributes;
#endif
};
} // namespace pdns::trace::dnsdist
