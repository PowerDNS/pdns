...
private:
  ...
  //
  // considerations:
  // * wheel_bits+(wheel_depth-1)*wheel_clockShift must be bigger than 32
  ...
  // used for delta 2^(wheel_bits+wheel_clockShift)=64..2^(wheel_bits+2*wheel_clockShift)-1=4095s
  ...
  public:
    ...
    Node(uint32_t key, time_t timerwheel_expiry, V value) :
      d_key(key), d_timerwheel_expiry(timerwheel_expiry), d_value(std::move(value)) {}
  ...
  time_t d_timerwheel_lastProcessed;
