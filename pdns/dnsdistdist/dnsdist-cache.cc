  ...
  d_settings(std::move(settings))
{
  ...
  for (auto& shard : d_shards) {
    shard.init((d_settings.d_maxEntries / d_settings.d_shardCount));
  }
}
...
bool DNSDistPacketCache::insertLocked(DNSDistPacketCacheContainer<CacheValue>& map, uint32_t key, uint32_t ttl, CacheValue& newValue)
{
  ...
  auto nowMonotonic = getmonotonic();
  auto result = map.insert(key, ttl + nowMonotonic, newValue);
  ...
  // this copies deep into the container
  value = newValue;

  map.bump(key, ttl + nowMonotonic);
  return false;
}
...
void DNSDistPacketCache::insert(uint32_t key, const std::optional<Netmask>& subnet, uint16_t queryFlags, bool dnssecOK, const DNSName& qname, uint16_t qtype, uint16_t qclass, const PacketBuffer& response, bool receivedOverUDP, uint8_t rcode, std::optional<uint32_t> tempFailureTTL)
{
  ...
  if (d_settings.d_deferrableInsertLock) {
    ...
    inserted = insertLocked(*lock, key, minTTL, newValue);
  }
  else {
...
size_t DNSDistPacketCache::purgeExpired(size_t upTo, const time_t now)
{
  ...
  for (auto& shard : d_shards) {
    ...
    auto shardRemoved = map->purgeExpired(toRemove, now);
