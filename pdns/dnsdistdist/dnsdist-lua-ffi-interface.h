...
/* Before 2.2 we required "addr" to be a large enough buffer to hold the 4 bytes of a IPv4 address, and "addrSize" was only an out parameter, but this triggered a lot of AI-assisted reports.
   So "addrSize" now has to be set to the size of the buffer pointed to by "addr", and the call will fail if the buffer is not large enough. "addrSize" is still
...
/* Before 2.2 we required "addr" to be a large enough buffer to hold the 16 bytes of a IPv6 address, and "addrSize" was only an out parameter, , but this triggered a lot of AI-assisted reports.
   So "addrSize" now has to be set to the size of the buffer pointed to by "addr", and the call will fail if the buffer is not large enough. "addrSize" is still
   updated to the number of bytes actually written to "addr" when the call succeeds. */
...
/* Before 2.2 we required "name" to be a large enough buffer to hold a DNS name, and "nameSize" was only an out parameter, but this triggered a lot of AI-assisted reports.
   So "nameSize" now has to be set to the size of the buffer pointed to by "name", and the call will fail if the buffer is not large enough. "nameSize" is still
