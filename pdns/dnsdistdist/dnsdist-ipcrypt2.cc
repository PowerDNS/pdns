...
IPCrypt2::IPCrypt2(const IPCryptMethod method, const std::string& key)
{
  ...
  case IPCryptMethod::pfx:
    ...
    d_ipcryptCtxPfx = std::unique_ptr<struct IPCryptPFX>(new IPCryptPFX);
    ipcrypt_pfx_init(d_ipcryptCtxPfx.get(), reinterpret_cast<const uint8_t*>(key.data()));
    break;
  default:
...
ComboAddress IPCrypt2::encrypt(const ComboAddress& address) const
{
  ...
  case IPCryptMethod::pfx: {
    ...
    auto ret = ipcrypt_pfx_encrypt_ip_str(d_ipcryptCtxPfx.get(), reinterpret_cast<char*>(&encryptedIP[0]), address.toString().c_str());
