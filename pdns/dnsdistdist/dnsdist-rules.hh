...
public:
  MaxQPSIPRule(unsigned int qps, unsigned int burst, unsigned int ipv4trunc = 32, unsigned int ipv6trunc = 64, unsigned int expiration = 300, unsigned int cleanupDelay = 60, unsigned int scanFraction = 10) :
    ...
    d_cleanupDelay(cleanupDelay),
    d_expiration(expiration),
    d_scanFraction(scanFraction)
  {
...
{
public:
  NMGRule(const NetmaskGroup& nmg) :
    d_nmg(nmg) {}
...
{
public:
  NotRule(shared_ptr<DNSRule>& rule) :
    d_rule(rule)
  {
  }
  bool matches(const DNSQuestion* dq) const override
...
{
public:
  KeyValueStoreLookupRule(std::shared_ptr<KeyValueStore>& kvs, std::shared_ptr<KeyValueLookupKey>& lookupKey) :
    d_kvs(kvs),
    d_key(lookupKey)
  {
  }
