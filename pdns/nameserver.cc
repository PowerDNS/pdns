...
void UDPNameserver::bindAddress()
{
  int one = 1;

  int s;

  ComboAddress locala(d_address, ::arg().asNum("local-port"));
...
bool UDPNameserver::receive(DNSPacket& packet, std::string& buffer)
{
  ...
  if((len=recvmsg(d_socket, &msgh, 0)) < 0 ) {
    if(errno != EAGAIN)
