...
void GSQLBackend::setNotified(uint32_t domain_id, uint32_t serial)
{
  try {
    ...
    d_UpdateSerialOfZoneQuery_stmt->bind("serial", serial)->bind("domain_id", domain_id)->execute()->reset();
  }
...
void GSQLBackend::setFresh(uint32_t domain_id)
{
  try {
    ...
    d_UpdateLastCheckofZoneQuery_stmt->bind("last_check", time(0))->bind("domain_id", domain_id)->execute()->reset();
  }
  catch (SSqlException& e) {
    throw PDNSException("GSQLBackend unable to refresh domain_id " + itoa(domain_id) + ": " + e.txtReason());
  }
...
bool GSQLBackend::setMaster(const DNSName& domain, const string& ip)
{
  try {
    ...
    d_UpdateMasterOfZoneQuery_stmt->bind("master", ip)->bind("domain", domain)->execute()->reset();
  }
  catch (SSqlException& e) {
...
bool GSQLBackend::setKind(const DNSName& domain, const DomainInfo::DomainKind kind)
{
  try {
    ...
    d_UpdateKindOfZoneQuery_stmt->bind("kind", toUpper(DomainInfo::getKindString(kind)))->bind("domain", domain)->execute()->reset();
  }
  catch (SSqlException& e) {
...
bool GSQLBackend::setAccount(const DNSName& domain, const string& account)
{
  try {
    ...
    d_UpdateAccountOfZoneQuery_stmt->bind("account", account)->bind("domain", domain)->execute()->reset();
  }
  catch (SSqlException& e) {
...
bool GSQLBackend::getDomainInfo(const DNSName& domain, DomainInfo& di, bool getSerial)
{
  ...
  try {
    ...
    d_InfoOfDomainsZoneQuery_stmt->bind("domain", domain)->execute()->getResult(d_result)->reset();
  }
  catch (SSqlException& e) {
...
void GSQLBackend::getUnfreshSlaveInfos(vector<DomainInfo>* unfreshDomains)
{
  ...
  try {
    ...
    d_InfoOfAllSlaveDomainsQuery_stmt->execute()->getResult(d_result)->reset();
  }
  catch (SSqlException& e) {
...
void GSQLBackend::getUpdatedMasters(vector<DomainInfo>* updatedDomains)
{
  ...
  try {
    ...
    d_InfoOfAllMasterDomainsQuery_stmt->execute()->getResult(d_result)->reset();
  }
  catch (SSqlException& e) {
  ...
  if (!ordername.empty()) {
    if (qtype == QType::ANY) {
      try {
        ...
        d_updateOrderNameAndAuthQuery_stmt->bind("ordername", ordername.labelReverse().toString(" ", false))->bind("auth", auth)->bind("domain_id", domain_id)->bind("qname", qname)->execute()->reset();
      }
      catch (SSqlException& e) {
      ...
      try {
        ...
        d_updateOrderNameAndAuthTypeQuery_stmt->bind("ordername", ordername.labelReverse().toString(" ", false))->bind("auth", auth)->bind("domain_id", domain_id)->bind("qname", qname)->bind("qtype", QType(qtype).getName())->execute()->reset();
      }
      catch (SSqlException& e) {
    ...
    if (qtype == QType::ANY) {
      ...
      try {
        d_nullifyOrderNameAndUpdateAuthQuery_stmt->bind("auth", auth)->bind("domain_id", domain_id)->bind("qname", qname)->execute()->reset();
      }
      catch (SSqlException& e) {
      ...
      try {
        ...
        d_nullifyOrderNameAndUpdateAuthTypeQuery_stmt->bind("auth", auth)->bind("domain_id", domain_id)->bind("qname", qname)->bind("qtype", QType(qtype).getName())->execute()->reset();
      }
      catch (SSqlException& e) {
...
bool GSQLBackend::updateEmptyNonTerminals(uint32_t domain_id, set<DNSName>& insert, set<DNSName>& erase, bool remove)
{
  if (remove) {
    try {
      ...
      d_RemoveEmptyNonTerminalsFromZoneQuery_stmt->bind("domain_id", domain_id)->execute()->reset();
    }
    catch (SSqlException& e) {
    ...
    for (const auto& qname : erase) {
      try {
        ...
        d_DeleteEmptyNonTerminalQuery_stmt->bind("domain_id", domain_id)->bind("qname", qname)->execute()->reset();
      }
      catch (SSqlException& e) {
  ...
  for (const auto& qname : insert) {
    try {
      ...
      d_InsertEmptyNonTerminalOrderQuery_stmt->bind("domain_id", domain_id)->bind("qname", qname)->bindNull("ordername")->bind("auth", true)->execute()->reset();
    }
    catch (SSqlException& e) {
...
bool GSQLBackend::getBeforeAndAfterNamesAbsolute(uint32_t id, const DNSName& qname, DNSName& unhashed, DNSName& before, DNSName& after)
{
  ...
  try {
    ...
    d_afterOrderQuery_stmt->bind("ordername", qname.labelReverse().toString(" ", false))->bind("domain_id", id)->execute();
  ...
  if (after.empty()) {
    try {
      ...
      d_firstOrderQuery_stmt->bind("domain_id", id)->execute();
  ...
  if (before.empty()) {
    ...
    try {
      ...
      d_beforeOrderQuery_stmt->bind("ordername", qname.labelReverse().toString(" ", false))->bind("domain_id", id)->execute();
    ...
    try {
      ...
      d_lastOrderQuery_stmt->bind("domain_id", id)->execute();
...
bool GSQLBackend::addDomainKey(const DNSName& name, const KeyData& key, int64_t& id)
{
  ...
  try {
    ...
    d_AddDomainKeyQuery_stmt->bind("flags", key.flags)->bind("active", key.active)->bind("published", key.published)->bind("content", key.content)->bind("domain", name)->execute()->reset();
  }
  catch (SSqlException& e) {
...
bool GSQLBackend::removeDomainKey(const DNSName& name, unsigned int id)
{
  ...
  try {
    ...
    d_RemoveDomainKeyQuery_stmt->bind("domain", name)->bind("key_id", id)->execute()->reset();
  }
  catch (SSqlException& e) {
...
bool GSQLBackend::getDomainKeys(const DNSName& name, std::vector<KeyData>& keys)
{
  ...
  try {
    ...
    d_ListDomainKeysQuery_stmt->bind("domain", name)->execute();
...
bool GSQLBackend::setDomainMetadata(const DNSName& name, const std::string& kind, const std::vector<std::string>& meta)
{
  ...
  try {
    ...
    d_ClearDomainMetadataQuery_stmt->bind("domain", name)->bind("kind", kind)->execute()->reset();
    if (!meta.empty()) {
      for (const auto& value : meta) {
        d_SetDomainMetadataQuery_stmt->bind("kind", kind)->bind("content", value)->bind("domain", name)->execute()->reset();
      }
    }
  }
  catch (SSqlException& e) {
  ...
  try {
    ...
    if (qtype.getCode() != QType::ANY) {
      if (domain_id < 0) {
        ...
        (*d_query_stmt)->bind("qtype", qtype.getName())->bind("qname", qname);
      }
      else {
        d_query_name = "id-query";
        d_query_stmt = &d_IdQuery_stmt;
        (*d_query_stmt)->bind("qtype", qtype.getName())->bind("qname", qname)->bind("domain_id", domain_id);
      }
    }
    else {
      // qtype==ANY
      if (domain_id < 0) {
        ...
        (*d_query_stmt)->bind("qname", qname);
      }
      else {
        d_query_name = "any-id-query";
        d_query_stmt = &d_ANYIdQuery_stmt;
        (*d_query_stmt)->bind("qname", qname)->bind("domain_id", domain_id);
      }
    }

    (*d_query_stmt)->execute();
  }
  catch (SSqlException& e) {
...
bool GSQLBackend::list(const DNSName& target, int domain_id, bool include_disabled)
{
  ...
  try {
    ...
    (*d_query_stmt)->bind("include_disabled", (int)include_disabled)->bind("domain_id", domain_id)->execute();
  }
  catch (SSqlException& e) {
...
bool GSQLBackend::listSubZone(const DNSName& zone, int domain_id)
{
  ...
  try {
    ...
    (*d_query_stmt)->bind("zone", zone)->bind("wildzone", wildzone)->bind("domain_id", domain_id)->execute();
  }
  catch (SSqlException& e) {
...
bool GSQLBackend::superMasterBackend(const string& ip, const DNSName& domain, const vector<DNSResourceRecord>& nsset, string* nameserver, string* account, DNSBackend** ddb)
{
  ...
  for (vector<DNSResourceRecord>::const_iterator i = nsset.begin(); i != nsset.end(); ++i) {
    try {
      ...
      d_SuperMasterInfoQuery_stmt->bind("ip", ip)->bind("nameserver", i->content)->execute()->getResult(d_result)->reset();
    }
    catch (SSqlException& e) {
...
bool GSQLBackend::createDomain(const DNSName& domain, const string& type, const string& masters, const string& account)
{
  try {
    ...
    d_InsertZoneQuery_stmt->bind("type", type)->bind("domain", domain)->bind("masters", masters)->bind("account", account)->execute()->reset();
  }
  catch (SSqlException& e) {
...
bool GSQLBackend::createSlaveDomain(const string& ip, const DNSName& domain, const string& nameserver, const string& account)
{
  ...
  try {
    if (!nameserver.empty()) {
      ...
      d_GetSuperMasterIPs_stmt->bind("nameserver", nameserver)->bind("account", account)->execute()->getResult(d_result)->reset();
      if (!d_result.empty()) {
...
bool GSQLBackend::deleteDomain(const DNSName& domain)
{
  ...
  try {
    ...
    d_DeleteZoneQuery_stmt->bind("domain_id", di.id)->execute()->reset();
    d_ClearDomainAllMetadataQuery_stmt->bind("domain", domain)->execute()->reset();
    d_ClearDomainAllKeysQuery_stmt->bind("domain", domain)->execute()->reset();
    d_DeleteCommentsQuery_stmt->bind("domain_id", di.id)->execute()->reset();
    d_DeleteDomainQuery_stmt->bind("domain", domain)->execute()->reset();
  }
  catch (SSqlException& e) {
...
void GSQLBackend::getAllDomains(vector<DomainInfo>* domains, bool include_disabled)
{
  ...
  try {
    ...
    d_getAllDomainsQuery_stmt->bind("include_disabled", (int)include_disabled)->execute();
...
bool GSQLBackend::replaceRRSet(uint32_t domain_id, const DNSName& qname, const QType& qt, const vector<DNSResourceRecord>& rrset)
{
  try {
    ...
    if (qt != QType::ANY) {
      d_DeleteRRSetQuery_stmt->bind("domain_id", domain_id)->bind("qname", qname)->bind("qtype", qt.getName())->execute()->reset();
    }
    else {
      d_DeleteNamesQuery_stmt->bind("domain_id", domain_id)->bind("qname", qname)->execute()->reset();
    }
  }
  catch (SSqlException& e) {
  ...
  if (rrset.empty()) {
    try {
      ...
      d_DeleteCommentRRsetQuery_stmt->bind("domain_id", domain_id)->bind("qname", qname)->bind("qtype", qt.getName())->execute()->reset();
    }
    catch (SSqlException& e) {
...
bool GSQLBackend::feedRecord(const DNSResourceRecord& r, const DNSName& ordername, bool ordernameIsNSEC3)
{
  ...
  try {
    ...
    d_InsertRecordQuery_stmt->bind("content", content)->bind("ttl", r.ttl)->bind("priority", prio)->bind("qtype", r.qtype.getName())->bind("domain_id", r.domain_id)->bind("disabled", r.disabled)->bind("qname", r.qname);
    ...
    d_InsertRecordQuery_stmt->execute()->reset();
  }
  catch (SSqlException& e) {
...
bool GSQLBackend::feedEnts(int domain_id, map<DNSName, bool>& nonterm)
{
  for (const auto& nt : nonterm) {
    try {
      ...
      d_InsertEmptyNonTerminalOrderQuery_stmt->bind("domain_id", domain_id)->bind("qname", nt.first)->bindNull("ordername")->bind("auth", (nt.second || !d_dnssecQueries))->execute()->reset();
    }
    catch (SSqlException& e) {
...
bool GSQLBackend::feedEnts3(int domain_id, const DNSName& domain, map<DNSName, bool>& nonterm, const NSEC3PARAMRecordContent& ns3prc, bool narrow)
{
  ...
  for (const auto& nt : nonterm) {
    try {
      ...
      d_InsertEmptyNonTerminalOrderQuery_stmt->bind("domain_id", domain_id)->bind("qname", nt.first);
      if (narrow || !nt.second) {
        d_InsertEmptyNonTerminalOrderQuery_stmt->bindNull("ordername");
      }
      else {
        ordername = toBase32Hex(hashQNameWithSalt(ns3prc, nt.first));
        d_InsertEmptyNonTerminalOrderQuery_stmt->bind("ordername", ordername);
      }
      d_InsertEmptyNonTerminalOrderQuery_stmt->bind("auth", nt.second)->execute()->reset();
    }
    catch (SSqlException& e) {
...
bool GSQLBackend::startTransaction(const DNSName& domain, int domain_id)
{
  try {
    ...
    if (domain_id >= 0) {
      d_DeleteZoneQuery_stmt->bind("domain_id", domain_id)->execute()->reset();
    }
  }
  catch (SSqlException& e) {
...
bool GSQLBackend::listComments(const uint32_t domain_id)
{
  try {
    ...
    (*d_query_stmt)->bind("domain_id", domain_id)->execute();
  }
  catch (SSqlException& e) {
...
void GSQLBackend::feedComment(const Comment& comment)
{
  try {
    ...
    d_InsertCommentQuery_stmt->bind("domain_id", comment.domain_id)->bind("qname", comment.qname)->bind("qtype", comment.qtype.getName())->bind("modified_at", comment.modified_at)->bind("account", comment.account)->bind("content", comment.content)->execute()->reset();
  }
  catch (SSqlException& e) {
...
bool GSQLBackend::searchRecords(const string& pattern, int maxResults, vector<DNSResourceRecord>& result)
{
  ...
  try {
    ...
    d_SearchRecordsQuery_stmt->bind("value", escaped_pattern)->bind("value2", escaped_pattern)->bind("limit", maxResults)->execute();
...
bool GSQLBackend::searchComments(const string& pattern, int maxResults, vector<Comment>& result)
{
  ...
  try {
    ...
    d_SearchCommentsQuery_stmt->bind("value", escaped_pattern)->bind("value2", escaped_pattern)->bind("limit", maxResults)->execute();
