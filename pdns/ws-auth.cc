...
static void gatherRecords(UeberBackend& B, const string& logprefix, const Json container, const DNSName& qname, const QType qtype, const int ttl, vector<DNSResourceRecord>& new_records, vector<DNSResourceRecord>& new_ptrs)
{
  ...
  for (const auto& record : items) {
    ...
    if ((rr.qtype.getCode() == QType::A || rr.qtype.getCode() == QType::AAAA) && boolFromJson(record, "set-ptr", false) == true) {
...
static void apiServerZoneExport(HttpRequest* req, HttpResponse* resp)
{
  ...
  while (di.backend->get(rr)) {
    ...
    ss << rr.qname.toString() << "\t" << rr.ttl << "\t"
       << "IN"
       << "\t" << rr.qtype.getName() << "\t" << makeApiRecordContent(rr.qtype, rr.content) << endl;
  }
