...
static void checkZoneTLSA(const set<DNSName>& tlsas, const set<DNSName>& cnames, const set<DNSName>& noncnames, std::vector<Check::diag>& diagnostics)
{
...
static void checkZoneSVCB(QType type, const ZoneName& zone, const svcbset_t& targets, const set<DNSName>& aliases, const set<DNSName>& records, const set<DNSName>& arecords, const set<DNSName>& aaaarecords, const set<DNSName>& addresses, std::vector<Check::diag>& diagnostics)
{
...
static void normalizeSOARecord(DNSResourceRecord& drr, std::vector<Check::diag>& diagnostics)
{
  vector<string> parts;
...
static bool checkRecordContents(DNSResourceRecord& drr, std::vector<Check::diag>& diagnostics)
{
...
static void checkNSEC3Params(Logr::log_t slog, UeberBackend& ueber, DNSSECKeeper& dsk, NSEC3PARAMRecordContent& ns3pr, bool haveNSEC3, bool isSecure, const ZoneName& zone, std::vector<Check::diag>& diagnostics)
{
...
static void checkParentDelegation(UeberBackend& ueber, const ZoneName& zone, std::vector<Check::diag>& diagnostics)
{
...
static void checkZoneRecords(std::vector<DNSResourceRecord>& records, const ZoneName& zone, Check::RRSetFlags flags, bool isExistingZone, bool canDoDNSSEC, bool presigned, bool isSecure, bool isOptOut, std::vector<Check::diag>& diagnostics)
{
...
static void checkZoneMetadata(UeberBackend& ueber, const ZoneName& zone, std::vector<Check::diag>& diagnostics)
{
...
static void checkZone(Logr::log_t slog, std::vector<DNSResourceRecord>& allrrs, const ZoneName& zone, Check::RRSetFlags flags, std::vector<Check::diag>& diagnostics)
{
  UeberBackend ueber;
...
static int checkZoneRecords(UeberBackend &B, const ZoneName& zone, const vector<DNSResourceRecord>* suppliedrecords=nullptr) // NOLINT(readability-identifier-length)
{
  int numerrors=0;
  int numwarnings=0;
  ...
  // Get zone contents from backend if not provided by caller.
...
static int checkAllZones(bool exitOnError)
{
  UtilBackend B("default"); //NOLINT(readability-identifier-length)
  vector<DomainInfo> domainInfo;
  ...
  for (auto& di : domainInfo) {
    if (checkZoneRecords(B, di.zone) > 0) {
      errors++;
    }
  ...
  while (true) {
    ...
    case EDITFILE:
      ...
      }
      {
        ...
        if(checkZoneRecords(B, zone, &checkrr) != 0) {
          state = INVALIDZONE;
          break;
        }
      }
...
static int checkZone(vector<string>& cmds, const std::string_view synopsis)
{
  ...
  UtilBackend B("default"); // NOLINT(readability-identifier-length)
  return checkZoneRecords(B, ZoneName(cmds.at(0)));
}
...
static int checkAllZones(vector<string>& cmds, [[maybe_unused]] const std::string_view synopsis)
{
  ...
  return checkAllZones(exitOnError);
}
