...
try {
  ...
  desc.add_options()("help,h", "produce help message")("version", "show version")("verbose,v", "be verbose")("force", "force an action")("config-name", po::value<string>()->default_value(""), "virtual configuration name")("config-dir", po::value<string>()->default_value(SYSCONFDIR), "location of pdns.conf")("commands", po::value<vector<string>>());
  ...
  else if(cmds[0] == "signing-slave")
  {
    ...
    if (!showZone(dk, DNSName(cmds[1]), true))
      return 1;
  }
    ...
    for (DNSBackend* b : BackendMakers().all()) {
      if (b->getPrefix() == cmds[1])
        src = b;
      if (b->getPrefix() == cmds[2])
        tgt = b;
    }
    if (!src) {
