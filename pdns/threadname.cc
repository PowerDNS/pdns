...
void setThreadName(const std::string& threadName)
{
  ...
  if (retval != 0) {
#ifdef DNSDIST
    SLOG(warnlog("Could not set thread name %s for thread: %s", threadName, stringerror(retval).c_str()),
...
#else
    SLOG(g_log << Logger::Warning << "Could not set thread name " << threadName << " for thread: " << stringerror(retval) << endl,
