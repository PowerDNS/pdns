...
public:
  OPTRecordContent() {}
  includeboilerplate(OPT) void getData(vector<pair<uint16_t, string>>& opts);
...
public:
  ...
  {
    return tie(d_flags, d_protocol, d_algorithm, d_key) < tie(rhs.d_flags, rhs.d_protocol, rhs.d_algorithm, rhs.d_key);
  }
};
...
public:
  ...
  bool operator==(const DNSRecordContent& rhs) const override
  {
    if (typeid(*this) != typeid(rhs))
      return false;
    auto rrhs = dynamic_cast<const decltype(this)>(&rhs);
    return tie(d_tag, d_algorithm, d_digesttype, d_digest) == tie(rrhs->d_tag, rrhs->d_algorithm, rrhs->d_digesttype, rrhs->d_digest);
  }
  bool operator<(const DSRecordContent& rhs) const
  {
    return tie(d_tag, d_algorithm, d_digesttype, d_digest) < tie(rhs.d_tag, rhs.d_algorithm, rhs.d_digesttype, rhs.d_digest);
  }
...
public:
  ...
  NSECBitmap(NSECBitmap&& rhs) :
    d_bitset(std::move(rhs.d_bitset)),
    d_set(std::move(rhs.d_set))
  {
  }
...
#define boilerplate_conv(RNAME, TYPE, CONV)                                                                         \
  boilerplate(RNAME, TYPE) template <class Convertor>                                                               \
