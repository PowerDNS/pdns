...
/* DNSPacket */
  ...
  d_lw->registerFunction<DNSPacket, ComboAddress()>("getLocal", [](DNSPacket &p) { return p.getInnerLocal(); }); // NOLINT(readability-identifier-length)
...
bool AuthLua4::updatePolicy(const DNSName &qname, const QType& qtype, const DNSName &zonename, const DNSPacket& packet) {
  ...
  upq.local = packet.getInnerLocal();
