...
{
public:
  DOHResponseMapEntry(const std::string& regex, uint16_t status, const PacketBuffer& content, const std::optional<std::vector<std::pair<std::string, std::string>>>& headers): d_regex(regex), d_customHeaders(headers), d_content(content), d_status(status)
  {
  ...
  const std::optional<std::vector<std::pair<std::string, std::string>>>& getHeaders() const
  {
...
private:
  Regex d_regex;
  std::optional<std::vector<std::pair<std::string, std::string>>> d_customHeaders;
