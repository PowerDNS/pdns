...
public:
  explicit OpenSSLRSADNSCryptoKeyEngine(unsigned int algo) :
    ...
    d_key(std::unique_ptr<RSA, void (*)(RSA*)>(nullptr, RSA_free))
  {
    int ret = RAND_status();
    if (ret != 1) {
...
public:
  explicit OpenSSLECDSADNSCryptoKeyEngine(unsigned int algo) :
    DNSCryptoKeyEngine(algo),
    d_eckey(std::unique_ptr<EC_KEY, void (*)(EC_KEY*)>(EC_KEY_new(), EC_KEY_free)),
    d_ecgroup(std::unique_ptr<EC_GROUP, void (*)(EC_GROUP*)>(nullptr, EC_GROUP_clear_free))
  {
...
public:
  explicit OpenSSLEDDSADNSCryptoKeyEngine(unsigned int algo) :
    DNSCryptoKeyEngine(algo),
    d_edkey(std::unique_ptr<EVP_PKEY, void (*)(EVP_PKEY*)>(nullptr, EVP_PKEY_free))
  {
