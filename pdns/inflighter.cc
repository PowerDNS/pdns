...
public:
  ...
  bool run(int cookie = 0); //!< keep calling this as long as it returns 1, or if it throws an exception 
...
template<typename Container, typename SendReceive> bool Inflighter<Container, SendReceive>::run(int cookie)
{
  if(!d_init)
    init();
  ...
  for(;;) {
    ...
    while(d_iter != d_container.end() && d_ttdWatch.size() < d_maxInFlight) { 
      TTDItem ttdi;
      ttdi.iter = d_iter++;
      ttdi.id = d_sr.send(*ttdi.iter, cookie);
...
//        cerr<<"DUPLICATE INSERT!"<<endl;
    ...
    if(!d_ttdWatch.empty()) {
      ...
      while(d_sr.receive(id, answer, cookie)) {
        ...
        if(ival != d_ttdWatch.end()) { // found something!
          ...
          d_sr.deliverAnswer(*ival->iter, answer, usec, cookie);    // deliver to sender/receiver
...
struct SendReceive
{
  ...
  Identifier send(int& i, int /*cookie*/)
  {
  ...
  bool receive(Identifier& id, int& i, int /*cookie*/)
  {
  ...
  void deliverAnswer(int& i, int j, int /*cookie*/)
  {
