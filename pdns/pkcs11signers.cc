...
static std::map<unsigned int, CK_MECHANISM_TYPE> dnssec2smech = boost::assign::map_list_of(5, CKM_SHA1_RSA_PKCS)(7, CKM_SHA1_RSA_PKCS)(8, CKM_SHA256_RSA_PKCS)(10, CKM_SHA512_RSA_PKCS)(13, CKM_ECDSA)(14, CKM_ECDSA);

// map for hashing algorithms
static std::map<unsigned int, CK_MECHANISM_TYPE> dnssec2hmech = boost::assign::map_list_of(5, CKM_SHA_1)(7, CKM_SHA_1)(8, CKM_SHA256)(10, CKM_SHA512)(13, CKM_SHA256)(14, CKM_SHA384);
...
private:
  ...
  unsigned int ecparam2bits(const std::string& obj) const
  {
    ...
    return bits;
#else
    if (d_ecdsa_params == "\x06\x08\x2a\x86\x48\xce\x3d\x03\x01\x07")
      return 256;
    else if (d_ecdsa_params == "\x06\x05\x2b\x81\x04\x00\x22")
      return 384;
    else
      throw PDNSException("Unsupported EC key");
#endif
...
public:
  ...
  int Sign(const std::string& data, std::string& result, CK_MECHANISM_PTR mechanism)
  {
    ...
    // perform signature
    if ((d_err = this->d_slot->f()->C_SignInit(d_slot->Session(), mechanism, d_private_key))) {
      logError("C_SignInit");
      return d_err;
    }
  ...
  int Verify(const std::string& data, const std::string& signature, CK_MECHANISM_PTR mechanism)
  {
    ...
    if ((d_err = this->d_slot->f()->C_VerifyInit(d_slot->Session(), mechanism, d_public_key))) {
      logError("C_VerifyInit");
      return d_err;
    }
  ...
  int Digest(const std::string& data, std::string& result, CK_MECHANISM_PTR mechanism)
  {
    ...
    if ((d_err = this->d_slot->f()->C_DigestInit(d_slot->Session(), mechanism))) {
...
std::shared_ptr<Pkcs11Token> Pkcs11Token::GetToken(const std::string& module, const string& tokenId, const std::string& label, const std::string& pub_label)
{
  ...
  if ((tokenIter = pkcs11_tokens.find(tidx)) != pkcs11_tokens.end())
...
PKCS11DNSCryptoKeyEngine::PKCS11DNSCryptoKeyEngine(const PKCS11DNSCryptoKeyEngine& orig) :
  DNSCryptoKeyEngine(orig.d_algorithm) {}
...
std::string PKCS11DNSCryptoKeyEngine::sign(const std::string& msg) const
{
  ...
  if (mech.mechanism == CKM_ECDSA) {
    if (d_slot->Sign(this->hash(msg), result, &mech))
      throw PDNSException("Could not sign data");
  }
  else {
    if (d_slot->Sign(msg, result, &mech))
      throw PDNSException("Could not sign data");
  }
  return result;
};
...
DNSCryptoKeyEngine::storvector_t PKCS11DNSCryptoKeyEngine::convertToISCVector() const
{
  ...
  boost::assign::push_back(storvect)(make_pair("Algorithm", std::to_string(d_algorithm)))(make_pair("Engine", d_module))(make_pair("Slot", d_slot_id))(make_pair("PIN", d_pin))(make_pair("Label", d_label))(make_pair("PubLabel", d_pub_label));
  return storvect;
};
