...
bool PacketHandler::addCDNSKEY(DNSPacket& p, std::unique_ptr<DNSPacket>& r, SOAData &sd)
{
  ...
  d_dk.getPublishCDNSKEY(sd.qname,publishCDNSKEY);
  ...
  bool haveOne=false;
  DNSSECKeeper::keyset_t entryPoints = d_dk.getEntryPoints(sd.qname);
...
bool PacketHandler::addCDS(DNSPacket& p, std::unique_ptr<DNSPacket>& r, SOAData &sd)
{
  string publishCDS;
  d_dk.getPublishCDS(sd.qname, publishCDS);
  if (publishCDS.empty())
    return false;
  ...
  DNSSECKeeper::keyset_t keyset = d_dk.getEntryPoints(sd.qname);
...
void PacketHandler::emitNSEC3(DNSPacket& p, std::unique_ptr<DNSPacket>& r, const NSEC3PARAMRecordContent& ns3prc, const DNSName& name, const string& namehash, const string& nexthash, int mode)
{
  ...
  if(!name.empty()) {
    if (d_sd.qname == name) {
      ...
      if (p.qtype.getCode() != QType::CDS) n3rc.set(QType::CDS);
      if (p.qtype.getCode() != QType::CDNSKEY) n3rc.set(QType::CDNSKEY);
    }
...
/*
   ...
   mode 6 = Authenticated DNSSEC bootstrapping
*/
...
bool PacketHandler::tryAuthSignal(DNSPacket& p, std::unique_ptr<DNSPacket>& r, DNSName &target) {
  ...
  if(!d_dk.isSignalingZone(d_sd.qname)) {
    return false;
  }

  // Check that we're doing online signing in narrow mode (as we don't know next owner names)
  if(!d_dk.isSecuredZone(d_sd.qname) || d_dk.isPresigned(d_sd.qname)) {
    g_log << Logger::Warning << "Signaling zone '" << d_sd.qname << "' must be secured (but not presigned!); synthesis disabled (" << target << "/" << p.qtype << " from " << p.getRemoteString() << ")" << endl;
    return false;
  }
  string val;
  d_dk.getFromMeta(d_sd.qname, "NSEC3NARROW", val);
  if(val != "1") {
    g_log << Logger::Warning << "Signaling zone '" << d_sd.qname << "' must use NSEC3 narrow; synthesis disabled (" << target << "/" << p.qtype << ")" << " from " << p.getRemoteString() << ")" << endl;
    return false;
  }
  ...
  if(!(p.qtype.getCode() == QType::CDS || p.qtype.getCode() == QType::CDNSKEY)) {
    // We don't know at this point whether CDS/CDNSKEY exist, so let's add both
  ...
  SOAData zone_sd;
  DNSName zone = target.makeRelative(d_sd.qname);
  zone.chopOff();
  // Zone must exist, but need not be secured (could be using front-sign setup)
  if(!B.getAuth(zone, p.qtype, &zone_sd) || zone_sd.qname != zone) {
  ...
  if(!autoPublish) {
      DNSZoneRecord rr;
      B.lookup(p.qtype.getCode(), zone, zone_sd.domain_id, &p);
      while(B.get(rr)) {
        rr.dr.d_name = p.qdomain;
        r->addRecord(std::move(rr));
        haveOne=true;
