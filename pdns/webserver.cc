...
void WebServer::bind()
{
  if (filesystem::is_socket(d_listenaddress.c_str())) {
    ...
    if(err < 0 && errno!=ENOENT) {
      SLOG(g_log<<Logger::Error<<d_logprefix<<"Listening on HTTP socket failed, unable to remove existing socket at "<<d_listenaddress.c_str()<<endl,
           d_slog->error(Logr::Error, e.what(), "Listening on HTTP socket failed, unable to remove existing socket", "exception", d_listenaddress.c_str()));
      d_server = nullptr;
      return;
    }
  }
