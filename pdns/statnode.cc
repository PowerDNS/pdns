...
void StatNode::submit(const DNSName& domain, int rcode, unsigned int bytes, bool hit, const boost::optional<const ComboAddress&>& remote)
{
  ...
  auto last = tmp.end() - 1;
  children[*last].submit(last, tmp.begin(), g_rootdnsname, rcode, bytes, remote, 1, hit);
}
...
void StatNode::submit(std::vector<string>::const_iterator end, std::vector<string>::const_iterator begin, const DNSName& domain, int rcode, unsigned int bytes, const boost::optional<const ComboAddress&>& remote, unsigned int count, bool hit)
{
