...
public:
  ...
  struct Policy
  {
    ...
    Policy(PolicyKind kind, PolicyType type, int32_t ttl = 0, std::shared_ptr<PolicyZoneData> data = nullptr, const std::vector<std::shared_ptr<const DNSRecordContent>>& custom = {}) :
      d_zoneData(std::move(data)), d_ttl(ttl), d_kind(kind), d_type(type)
    {
      if (!custom.empty()) {
    ...
    Policy& operator=(Policy&& pol) = default;
    ~Policy() = default;
  ...
  class AppliedPolicy : public Policy
  {
  public:
    AppliedPolicy& operator=(const Policy& pol)
    {
      Policy::operator=(pol);
      d_kind = pol.d_kind;
      d_type = pol.d_type;
  ...
  public:
    ...
    void addQNameTrigger(const DNSName& netmask, Policy&& pol, bool ignoreDuplicate = false);
    void addNSTrigger(const DNSName& name, Policy&& pol, bool ignoreDuplicate = false);
    ...
    bool rmQNameTrigger(const DNSName& netmask, const Policy& pol);
    bool rmNSTrigger(const DNSName& name, const Policy& pol);
    bool rmNSIPTrigger(const Netmask& netmask, const Policy& pol);
    bool rmResponseTrigger(const Netmask& netmask, const Policy& pol);

    bool findExactQNamePolicy(const DNSName& qname, DNSFilterEngine::AppliedPolicy& pol) const;
    bool findExactNSPolicy(const DNSName& qname, DNSFilterEngine::AppliedPolicy& pol) const;
    bool findNSIPPolicy(const ComboAddress& addr, DNSFilterEngine::AppliedPolicy& pol) const;
    bool findResponsePolicy(const ComboAddress& addr, DNSFilterEngine::AppliedPolicy& pol) const;
    bool findClientPolicy(const ComboAddress& addr, DNSFilterEngine::AppliedPolicy& pol) const;
  ...
  private:
    ...
    static bool findExactNamedPolicy(const std::unordered_map<DNSName, DNSFilterEngine::Policy>& polmap, const DNSName& qname, DNSFilterEngine::AppliedPolicy& pol);
    static bool findNamedPolicy(const std::unordered_map<DNSName, DNSFilterEngine::Policy>& polmap, const DNSName& qname, DNSFilterEngine::AppliedPolicy& pol);
  ...
  bool getQueryPolicy(const DNSName& qname, const std::unordered_map<std::string, bool>& discardedPolicies, AppliedPolicy& policy) const;
  bool getClientPolicy(const ComboAddress& address, const std::unordered_map<std::string, bool>& discardedPolicies, AppliedPolicy& policy) const;
  bool getProcessingPolicy(const DNSName& qname, const std::unordered_map<std::string, bool>& discardedPolicies, AppliedPolicy& policy) const;
  bool getProcessingPolicy(const ComboAddress& address, const std::unordered_map<std::string, bool>& discardedPolicies, AppliedPolicy& policy) const;
  bool getPostPolicy(const vector<DNSRecord>& records, const std::unordered_map<std::string, bool>& discardedPolicies, AppliedPolicy& policy) const;
  bool getPostPolicy(const DNSRecord& record, const std::unordered_map<std::string, bool>& discardedPolicies, AppliedPolicy& policy) const;

  // A few convenience methods for the unit test code
  [[nodiscard]] AppliedPolicy getQueryPolicy(const DNSName& qname, const std::unordered_map<std::string, bool>& discardedPolicies, Priority prio) const
  {
    AppliedPolicy policy;
    policy.d_zoneData = std::make_shared<PolicyZoneData>();
    policy.d_zoneData->d_priority = prio;
    getQueryPolicy(qname, discardedPolicies, policy);
    return policy;
  }

  [[nodiscard]] AppliedPolicy getClientPolicy(const ComboAddress& address, const std::unordered_map<std::string, bool>& discardedPolicies, Priority prio) const
  {
    AppliedPolicy policy;
    policy.d_zoneData = std::make_shared<PolicyZoneData>();
    policy.d_zoneData->d_priority = prio;
  ...
  [[nodiscard]] AppliedPolicy getProcessingPolicy(const DNSName& qname, const std::unordered_map<std::string, bool>& discardedPolicies, Priority prio) const
  {
    AppliedPolicy policy;
    policy.d_zoneData = std::make_shared<PolicyZoneData>();
    policy.d_zoneData->d_priority = prio;
  ...
  [[nodiscard]] AppliedPolicy getProcessingPolicy(const ComboAddress& address, const std::unordered_map<std::string, bool>& discardedPolicies, Priority prio) const
  {
    AppliedPolicy policy;
    policy.d_zoneData = std::make_shared<PolicyZoneData>();
    policy.d_zoneData->d_priority = prio;
  ...
  [[nodiscard]] AppliedPolicy getPostPolicy(const vector<DNSRecord>& records, const std::unordered_map<std::string, bool>& discardedPolicies, Priority prio) const
  {
    AppliedPolicy policy;
    policy.d_zoneData = std::make_shared<PolicyZoneData>();
    policy.d_zoneData->d_priority = prio;
