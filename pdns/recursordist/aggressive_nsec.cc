...
size_t AggressiveNSECCache::ZoneEntry::CacheEntry::sizeEstimate() const
{
  ...
  if (d_record) {
    ret += d_record->serialize(g_rootdnsname, true).size();
  }
  for (const auto& entry : d_signatures) {
    if (entry) {
      ret += sizeof(RRSIGRecordContent);
      ret += entry->d_signer.wirelength();
      ret += entry->d_signature.size();
    }
  }
  ret += d_owner.wirelength();
  ret += d_next.wirelength();
  ret += d_qname.wirelength();
  return ret;
}
