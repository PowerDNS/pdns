...
void DNSFilterEngine::Zone::dump(FILE* fp) const
{
  if (DNSRecord soa = d_zoneData->d_soa; !soa.d_name.empty()) {
    ...
    /* fake the SOA record */
    auto soarr = DNSRecordContent::mastermake(QType::SOA, QClass::IN, "fake.RPZ. hostmaster.fake.RPZ. " + std::to_string(d_serial) + " " + std::to_string(d_refresh) + " 600 3600000 604800");
