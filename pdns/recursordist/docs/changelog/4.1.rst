...
.. changelog::
  ...
  .. change::
    ...
    Add ``disable-real-memory-usage`` switch to disable the collection
    of the real-memory-usage metric. It turns out that, on some kernel
    versions and with a lot of memory pages mapped, accessing
    ``/proc/self/smaps`` hurts.
