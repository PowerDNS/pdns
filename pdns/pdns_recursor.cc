...
static void startDoResolve(void *p)
{
  ...
  try {
    ...
    sr.setDNSSECValidationRequested(g_dnssecMode == DNSSECValidationMode::Validate || ((dc->d_mdp.d_header.ad || DNSSECOK) && g_dnssecMode==DNSSECValidationMode::ClientOnly));
...
static int serviceMain(int argc, char*argv[])
{
  ...
  if(pdns_iequals(::arg()["dnssec-validation"], "yes") || pdns_iequals(::arg()["dnssec-validation"], "on")) {
    g_dnssecMode = DNSSECValidationMode::Validate;
  } else if(pdns_iequals(::arg()["dnssec-validation"], "process")) {
    g_dnssecMode = DNSSECValidationMode::Process;
  } else if(pdns_iequals(::arg()["dnssec-validation"],  "no") || pdns_iequals(::arg()["dnssec-validation"], "off")) {
    g_dnssecMode = DNSSECValidationMode::Off;
  } else if(pdns_iequals(::arg()["dnssec-validation"], "client")) {
    g_dnssecMode = DNSSECValidationMode::ClientOnly;
  } else {
    g_log<<Logger::Error<<"Unknown DNSSEC Validation mode "<<::arg()["dnssec-validation"]<<endl;
    exit(1);
  }
  ...
  if(pdns_iequals(::arg()["dnssec-servfail"], "yes")) {
    ...
    g_log<<Logger::Error<<"Unknown DNSSEC BogusServfail mode "<<::arg()["dnssec-servfail"]<<endl;
    exit(1);
  }
  ...
  if (pdns_iequals(::arg()["dnssec-log"], "yes") || pdns_iequals(::arg()["dnssec-log"], "all")) {
    ...
    g_log<<Logger::Error<<"Unknown DNSSEC Logging mode "<<::arg()["dnssec-log"]<<endl;
    exit(1);
  }
...
int main(int argc, char **argv)
{
  ...
  try {
    ...
    ::arg().set("dnssec-servfail", "DNSEC send SERVFAIL on bogus: yes/no/client")="no";
    ::arg().set("dnssec-log", "DNSEC log validation results: yes/all/no/bogus")="no";
