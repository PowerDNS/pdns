...
static std::vector<std::unique_ptr<DNSBackend>>::iterator findBestMatchingBackend(std::shared_ptr<Logr::Logger> slog, std::vector<std::unique_ptr<DNSBackend>>& backends, std::vector<std::pair<std::size_t, SOAData>>& bestMatches, const ZoneName& shorter, SOAData* soaData)
{
...
static bool foundTarget([[maybe_unused]] std::shared_ptr<Logr::Logger> slog, const ZoneName& target, const ZoneName& shorter, const QType& qtype, SOAData* soaData, const bool found)
{
...
void UeberBackend::lookup(const QType& qtype, const DNSName& qname, domainid_t zoneId, DNSPacket* pkt_p)
{
  ...
  if (backends.empty()) {
    SLOG(g_log << Logger::Error << "No database backends available - unable to answer questions." << endl,
         d_slog->error(Logr::Error, "No database backends available - unable to answer questions"));
