    ...
    if (foundRecord) {
      ...
      if (rrType == QType::SOA) { // SOA updates require the serial to be higher than the current
        ...
        if (rfc1982LessThan(sdOld.serial, sdUpdate.serial)) {
          ...
          *updatedSerial = true;
          changedRecords++;
          g_log<<Logger::Notice<<msgPrefix<<"Replacing SOA record "<<rr->d_name<<"|"<<rrType.getName()<<endl;
        } else {
      ...
      // It's not possible to have multiple CNAME's with the same NAME. So we always update.
        ...
        if (changedCNames > 0) {
          ...
          g_log<<Logger::Notice<<msgPrefix<<"Replacing CNAME record "<<rr->d_name<<"|"<<rrType.getName()<<endl;
          changedRecords += changedCNames;
        } else {
          g_log<<Logger::Notice<<msgPrefix<<"Replace for CNAME record "<<rr->d_name<<"|"<<rrType.getName()<<" requested, but no changes made."<<endl;
        }
