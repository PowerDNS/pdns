...
static void apiWriteConfigFile(const string& filebasename, const string& content)
{
  if (::arg()["api-config-dir"].empty()) {
    throw ApiException(ApiException::ErrInvalidConfig, "Config Option \"api-config-dir\" must be set");
  }

  string filename = ::arg()["api-config-dir"] + "/" + filebasename + ".conf";
  ofstream ofconf(filename.c_str());
  if (!ofconf) {
    throw ApiException(ApiException::ErrIOError, "Could not open config fragment file '"+filename+"' for writing: "+stringerror());
  }
...
static void apiServerConfigAllowFrom(HttpRequest* req, HttpResponse* resp)
{
  if (req->method == "PUT") {
    ...
    if (!jlist.is_array()) {
      throw ApiException(ApiException::ErrInvalidInput, "'value' must be an array");
    }
    ...
    for (auto value : jlist.array_items()) {
      try {
        ...
        throw ApiException(ApiException::ErrGenericError, e.reason);
      }
    }
...
static void fillZone(const DNSName& zonename, HttpResponse* resp)
{
  ...
  if (iter == SyncRes::t_sstorage.domainmap->end())
    throw ApiException(ApiException::ErrZoneNotExists, "Could not find domain '"+zonename.toLogString()+"'");
...
static void doCreateZone(const Json document)
{
  ...
  if (kind == "NATIVE") {
    if (rd)
      throw ApiException(ApiException::ErrInvalidInput, "kind=Native and recursion_desired are mutually exclusive");
    if(!singleIPTarget.empty()) {
      try {
	ComboAddress rem(singleIPTarget);
	if(rem.sin4.sin_family != AF_INET)
	  // TODO: Add meaningful message here as well as replace generic error code to something more useful
	  throw ApiException(ApiException::ErrGenericError, "");
	singleIPTarget = rem.toString();
      }
      catch(...) {
	throw ApiException(ApiException::ErrInvalidInput, "Single IP target '"+singleIPTarget+"' is invalid");
      }
    }
    string zonefilename = ::arg()["api-config-dir"] + "/" + confbasename + ".zone";
    ofstream ofzone(zonefilename.c_str());
    if (!ofzone) {
      throw ApiException(ApiException::ErrIOError, "Could not open '"+zonefilename+"' for writing: "+stringerror());
    }
    ...
    for (auto value : document["servers"].array_items()) {
      ...
      if (server == "") {
        throw ApiException(ApiException::ErrInvalidInput, "Forwarded-to server must not be an empty string");
      }
      try {
    ...
    if (serverlist == "")
      throw ApiException(ApiException::ErrInvalidInput, "Need at least one upstream server when forwarding");
    ...
    throw ApiException(ApiException::ErrInvalidKind, "invalid kind");
  }
}
...
static void apiServerZones(HttpRequest* req, HttpResponse* resp)
{
  if (req->method == "POST") {
    ...
    auto iter = SyncRes::t_sstorage.domainmap->find(zonename);
    if (iter != SyncRes::t_sstorage.domainmap->end())
      throw ApiException(ApiException::ErrZoneAlreadyExists, "Zone already exists");
...
static void apiServerZoneDetail(HttpRequest* req, HttpResponse* resp)
{
  ...
  if(req->method == "PUT") {
    ...
    if (!doDeleteZone(zonename)) {
      throw ApiException(ApiException::ErrOPFailed, "Deleting domain failed");
    }
...
void RecursorWebServer::jsonstat(HttpRequest* req, HttpResponse *resp)
{
  ...
  if(command == "get-query-ring") {
    ...
    throw ApiException(ApiException::ErrGenericError, "Command '"+command+"' not found", 404);
  }
}
