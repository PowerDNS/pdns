...
shared_ptr<DNSRecordContent> DNSRecordContent::unserialize(const DNSName& qname, uint16_t qtype, const string& serialized)
{
  ...
  memcpy(&packet[0], &dnsheader, sizeof(dnsheader));
  pos += sizeof(dnsheader);

  char tmp[6] = "\x0"
                "\x0\x1"
                "\x0\x1"; // root question for ns_t_a
  memcpy(&packet[pos], &tmp, 5);
  pos += 5;

  memcpy(&packet[pos], encoded.c_str(), encoded.size());
  ...
  memcpy(&packet[pos], &drh, sizeof(drh));
  pos += sizeof(drh);
...
void MOADNSParser::init(bool query, const std::string& packet)
{
  ...
  try {
    ...
    for (n = 0; n < (unsigned int)(d_header.ancount + d_header.nscount + d_header.arcount); ++n) {
      ...
      if (query && !(d_qtype == QType::IXFR && dr.d_place == DNSResourceRecord::AUTHORITY && dr.d_type == QType::SOA) && // IXFR queries have a SOA in their AUTHORITY section
    ...
    if (validPacket && d_header.tc) { // don't sweat it over truncated packets, but do adjust an, ns and arcount
      if (n < d_header.ancount) {
        d_header.ancount = n;
        d_header.nscount = d_header.arcount = 0;
      }
      else if (n < d_header.ancount + d_header.nscount) {
        d_header.nscount = n - d_header.ancount;
        d_header.arcount = 0;
      }
      else {
      ...
      throw MOADNSException("Error parsing packet of " + std::to_string(packet.size()) + " bytes (rd=" + std::to_string(d_header.rd) + "), out of bounds: " + string(re.what()));
    }
  }
}
